...
function ChatWidget({ open, onClose, initialMessage }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 w-80 sm:w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 flex flex-col z-50 overflow-hidden"
      style={{ maxHeight: '70vh' }}>
      {/* header */}
      <div className="bg-gradient-to-r from-teal-600 to-green-600 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-white dark:bg-slate-800/20 flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50 dark:bg-slate-900">
        ...
        {typing && (
          <div className="flex justify-start">
            ...
            <div className="bg-white dark:bg-slate-800 border border-slate-100 px-4 py-2 rounded-2xl rounded-bl-sm shadow-sm flex gap-1 items-center">
              {[0, 0.2, 0.4].map((d, i) => (
      ...
      {/* input */}
      <div className="p-3 border-t border-slate-100 bg-white dark:bg-slate-800 flex gap-2">
        <input
          className="flex-1 bg-slate-50 dark:bg-slate-900 rounded-xl px-3 py-2 text-sm border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-400"
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4"
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="font-bold text-slate-900 dark:text-slate-100 text-lg">Call Us</h3>
        ...
        {[
          ...
          <a key={region} href={`tel:${number.replace(/\s/g, '')}`}
            ...
            <div className="flex-1">
              ...
              <p className="font-semibold text-slate-900 dark:text-slate-100 group-hover:text-teal-700 transition-colors">{number}</p>
            </div>
    ...
    <>
      ...
      <div className="bg-gradient-to-br from-teal-700 via-teal-600 to-green-600 pt-28 pb-16 px-4 text-white relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          {/* agent illustration */}
          <div className="hidden sm:flex w-32 h-32 rounded-full bg-white dark:bg-slate-800/10 border border-white/20 items-center justify-center text-7xl shrink-0">
            🎧
          </div>
        </div>
      </div>

      <div className="bg-slate-100 dark:bg-slate-800 min-h-screen -mt-4">
        <div className="max-w-4xl mx-auto px-4 pt-8 pb-16">

          {/* ── Service chat card ── */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-slate-100 p-6 sm:p-8 mb-4">
            {/* card header */}
            <div className="flex items-center gap-2 mb-5">
              <span className="w-3 h-3 bg-teal-500 rounded-full animate-pulse" />
              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Service chat</h2>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              {filteredFaqs.map(faq => (
                <div key={faq.id}>
                  <button
                    ...
                    className={`w-full text-left flex items-center justify-between gap-2 px-4 py-3.5 rounded-xl border transition-all
                      ${faq.isChat
                        ...
                        : 'border-slate-100 bg-slate-50 dark:bg-slate-900 hover:bg-teal-50 hover:border-teal-200 text-slate-700 dark:text-slate-300'}`}>
                  ...
                  {expandedFaq === faq.id && (
                    <div className="mt-1 px-4 py-3 bg-teal-50 border border-teal-100 rounded-xl text-sm text-slate-600 dark:text-slate-400">
                ...
                <button key={tag}
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all
                    ${activeFilter === tag
                      ...
                      : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 hover:border-teal-400 hover:text-teal-700'}`}>
                  {tag}
                </button>
              ))}
              <button onClick={() => setActiveFilter(null)}
                className="px-3 py-1.5 rounded-full text-xs font-semibold border border-slate-200 text-slate-400 hover:text-slate-600 dark:text-slate-400 transition-colors">
                ···
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            {[
              ...
              <button key={label} onClick={action}
                className={`bg-white dark:bg-slate-800 border border-slate-100 rounded-2xl p-4 flex flex-col items-center gap-2 shadow-sm hover:shadow-md hover:border-${color}-200 hover:-translate-y-0.5 transition-all group`}>
          ...
          {/* ── Guarantee banner ── */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-teal-100 shadow-sm p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-teal-50 flex items-center justify-center shrink-0">
              <i className="ph ph-shield-check text-teal-600 text-2xl" />
            </div>
            <div className="flex-1">
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-1">Travel worry-free with our reliable support</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">Your booking is protected against unexpected issues. Our team is available 24/7 to resolve any problems.</p>
            </div>
