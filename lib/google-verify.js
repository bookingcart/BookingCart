...
async function verifyAuthorizationBearer(bearerHeader) {
  ...
  try {
    ...
    // Not our JWT. Fall back to Google verification below.
  }
  ...
  try {
    ...
    const expired = /expired/i.test(String(e && e.message ? e.message : ''));
    return { ok: false, status: 401, error: expired ? 'Session expired. Please sign in again.' : 'Invalid authentication token' };
  }
}
