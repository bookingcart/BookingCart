...
function getAuthOrigin() {
  return normalizeOrigin(process.env.BETTER_AUTH_URL || process.env.APP_URL || '');
}

function getRequestOriginFromHeaders(headers, fallbackHost = '') {
  const source = headers || {};
  const direct = normalizeOrigin(source.origin || source.Origin || '');
  if (direct) return direct;

  const proto = String(source['x-forwarded-proto'] || source['X-Forwarded-Proto'] || 'http')
    .split(',')[0]
    .trim();
  const host = String(
...
function getAppOriginConfigError() {
  if (process.env.NODE_ENV === 'production' && !getAppOrigin()) {
    return 'APP_URL is required in production';
  }
  return null;
}

function getAuthOriginConfigError() {
  if (process.env.NODE_ENV === 'production' && !getAuthOrigin()) {
    return 'BETTER_AUTH_URL or APP_URL is required in production';
  }
  return null;
}
