...
async function checkPrices() {
  ...
  try {
    ...
    if (collections && collections.priceAlerts) {
      activeAlerts = await collections.priceAlerts.find({ status: 'active' }).toArray();
    }
  } catch (err) {
    console.warn('[Price Tracker] MongoDB query failed, checking memory fallback:', err.message);
  ...
  for (const alert of activeAlerts) {
    try {
      ...
      if (datesToCheck.length === 0) {
        console.warn(`[Price Tracker] No departure dates specified for alert ${alert._id || alert.email}. Skipping.`);
        continue;
      }
      ...
      if (cheapestOffer) {
        ...
        if (cheapestOffer.price <= alert.targetPrice) {
          ...
          if (collections && collections.priceAlerts && alert._id) {
            await collections.priceAlerts.updateOne(
              { _id: alert._id },
